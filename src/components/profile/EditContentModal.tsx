...
const EditContentModal = ({ open, onOpenChange, content, onSave }: EditContentModalProps) => {
  ...
  const form = useForm<EditFormValues>({
    ...
    values: content ? {
      ...
      visibility: content.visibility as 'Público' | 'Exclusivo assinantes' | 'Pago individualmente',
    } : undefined,
  });
  ...
  const handleSubmit = async (values: EditFormValues) => {
    ...
    try {
      ...
      const response = await contentService.updateContent(content._id, formData);
  ...
  const displayPreview = newPreview ? newPreview : `${basicUrl}img/contents/${content.url}`;
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            ...
            <div className="relative rounded-lg overflow-hidden bg-secondary/50 aspect-video">
              {form.watch("type") === "video" ? (
                <video src={displayPreview} className="h-full w-full object-cover" controls />
              ) : (
              ...
              <label className="absolute bottom-2 right-2 h-8 px-3 rounded-md bg-black/60 flex items-center gap-1.5 text-xs text-white cursor-pointer hover:bg-black/80 transition-colors">
                ...
                <input type="file" accept="image/*,video/*" className="hidden" onChange={(e) => e.target.files?.[0] && handleFileChange(e.target.files[0])} />
              </label>

              {newFile && (
                <button type="button" onClick={() => { setNewFile(null); setNewPreview(null); }} className="absolute top-2 right-2 h-6 w-6 rounded-full bg-black/60 flex items-center justify-center text-white">
                  <X className="h-3 w-3" />
                </button>
              )}
            </div>
